...
# 读取州区域映射文件
region_map = pd.read_csv('/DeepLearning/mnt/shixiansheng/data_fusion/output/Region/2011_299*459_StateRegions.csv')
region_map = region_map[['ROW', 'COL', 'StateRegion', 'StateRegionId']].dropna()
region_map['StateRegionId'] = region_map['StateRegionId'].astype(int)
# 排除不在任何州的区域
region_map = region_map[region_map['StateRegion'] != "-999"]  
...
# 提取2010年人口数据并重命名列
state_population = population_data.groupby('STATE_NAME')['POPULATION_2010'].sum().reset_index()
state_population.columns = ['StateRegion', 'Population']
...
climate_region_mapping = {
    ...
    'USA': list(state_population['StateRegion'].unique())  # USA包含所有州
}
...
# 为region_map添加气候区信息
region_map['ClimateRegion'] = region_map['StateRegion'].map(state_to_climate_region)
# 过滤掉不在映射中的州
...
# 创建输出目录
output_dir = '/DeepLearning/mnt/shixiansheng/data_fusion/output/9ClimateRegion_PopWeighted_Timeseries'
...
for period in tqdm(periods, desc="处理时间段"):
    ...
    for climate_region in tqdm(all_regions, desc="处理气候区", leave=False):
        ...
        # 创建区域数据（包含该气候区下的所有州）
        region_period_data = period_data[period_data['StateRegion'].isin(states_in_region)].copy()
        ...
        for i, method in enumerate(methods):
            if method in region_period_data.columns:
                ...
                for year in years:
                    ...
                    # 按州分组计算平均浓度
                    state_avg = year_data.groupby('StateRegion')[method].mean().reset_index()
                    
                    # 合并人口数据
                    state_avg = state_avg.merge(state_population, on='StateRegion', how='left')
                    
                    # 计算人口加权浓度
                    if not state_avg.empty and 'Population' in state_avg.columns and not state_avg['Population'].isna().all():
                        pop_weighted_concentration = (state_avg[method] * state_avg['Population']).sum() / state_avg['Population'].sum()
        ...
        filename = f"{output_dir}/{period}_{cmaq_name}_Ozone_PopWeighted_Timeseries.png"
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        plt.close()
        
print("所有人口加权时间序列图生成完成！")